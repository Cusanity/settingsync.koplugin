...
--
--[[--
A pure Lua implementation of a gettext subset for settingsync.koplugin.
--]]
...
function GetText_mt.__index.changeLang(new_lang)
    ...
    if not po then
        logger.dbg("settingsync: cannot open translation file:", file)
        return false
    end
    logger.info("settingsync: successfully opened translation file:", file)
    print("settingsync: successfully opened translation file:", file)
    ...
    while true do
        ...
        if line == nil or line == "" then
            ...
            if not line:match("^#") then
                ...
                elseif not (what and s == "" and fuzzy) then
                    fuzzy = false
                end
...
-- settingsync.koplugin specific setup
GetText.dirname = require("datastorage"):getDataDir() .. "/plugins/settingsync.koplugin/l10n"
...
local function loadCurrentLang()
    ...
    if not ok then
        logger.err("settingsync: ERROR in loadCurrentLang:", err)
    end
end
