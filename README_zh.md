...
- **不漏项** – 「其他阅读器设置」兜底类别会接管所有未被其他类别认领的键值；`settings/*.lua` 设置文件会自动扫描发现。因此 KOReader 升级新增的设置或新装插件的配置无需等待本插件更新即可同步
- **设备专属排除** – 按键名形态过滤路径类和每台设备独有的状态
- **云端设备一览** – **云端设备…** 列出云端文件夹中的所有设备配置及各自已备份的内容，点击某台设备即可查看其分组
...
账号、密码、Wi-Fi 凭据和 API 密钥**会**同步——这些正是新设备需要的配置。它们被归入各自的类别（**已保存的密码**、**已保存的 Wi-Fi 网络**、各插件的设置），默认开启，如不需要可在 **同步范围** 中关闭。请注意：这意味着云端目录以明文保存这些内容，请妥善保管 WebDAV 账号。
